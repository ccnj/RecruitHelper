...
- **平台交互节奏下限(2026-07-23 甲方裁决)。** 相邻的平台交互动作(点击、输入、滚动、关闭、切换等)必须至少间隔 1 秒；批量处理相邻候选人的随机等待下限为 2 秒，当前默认范围 2～4 秒；打开候选人简历后必须至少停留 2 秒才允许关闭。条件就绪/稳定等待仍须保留，固定时长下限不能替代后置条件确认。
