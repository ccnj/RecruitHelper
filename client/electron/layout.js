...
function resolveLayout(opts) {
  ...
  if (packaged) {
    ...
    if (!exists(brainBin)) {
      ...
      // 2026-08-10 客户机真机现场:文件缺失的实际成因是杀毒软件误报后隔离
      // 删除(Windows Defender 判 DCRat),不是安装器没写全 —— 提示必须把这个
      // 已证实的原因说给现场的人,否则只会反复重装、反复被隔离。
      throw new Error(
        `脑服务文件缺失:${brainBin}\n\n` +
          '可能被杀毒软件误报后隔离删除,也可能安装包不完整。\n' +
          '请先检查杀毒软件的拦截记录(Windows 安全中心 → 病毒和威胁防护 → ' +
          '保护历史记录):若有,请将安装目录加入排除项,再还原被隔离的文件;' +
          '若没有拦截记录,请重新安装本程序。',
      )
    }
    return {
