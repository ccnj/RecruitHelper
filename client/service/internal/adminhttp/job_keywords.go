...
// provider 调用失败、返回不是合法 JSON、数量越界、词重复、单组超配额这几种;
// 模型给出的合法选择一律采纳——真正的闸是发布前运营在二次确认清单上看得见
// 这几个词再确认。
...
//
// 五种失败都重试:provider 调用失败、返回不是合法 JSON、数量越界、词重复、
// 单组超配额。复核一步都不放宽——关键词决定平台把职位匹配给谁,选错的页面
// 看上去一切正常。
