...
type PlanWord struct {
	...
	// Passthrough:这个词是**键盘布局直接打出来**的,不经组字——字面数字、半角空格、
	// 半角标点、换行。TIP 收到这个标记就不吃那个键、只把词序号推进一格。
	//
	// 中文标点相反(走组字,真微软拼音如此),所以不能由 TIP 看键码自己判断:同一个
	// Comma 键可能是「，」也可能是 ","。只有排版器知道,本包同样只负责转交
	// (线格式见 tipwire.go)。上游 2026-09-01 真机实测:一个字面数字被当上屏键吃掉,
	// 离线模型与真机的五项对账就有四项对不上。
