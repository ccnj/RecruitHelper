...
// Reseed 把标定打回冷启动，并用当前的窗口粗估重新播一遍。
//
// **只在旧标定被证明没用时调。** 判据是"一次完整移动零观测"：光标压根不在页面上，
// 最常见的原因是窗口被拖到了另一块屏。那时旧映射把光标送到别的屏，页面收不到任何
// 事件 —— 而修正映射**必须**有观测。死循环，只有重新播种能出来。
//
// 它是**降级**(拿准的换成猜的)，所以不设任何自动触发：由编排层在零观测时显式调用。
// 重播之后仍然零观测，照旧停手 —— 失效方向不变。
func (s *Service) Reseed(h WindowHint) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.pb.SeedCalib(s.inj.SeedCalib(h))
	s.seeded = true
	// 标定没了，上一次的落点确认自然作废;lastInj 也清掉，免得下一个落点跟一个
	// 属于旧几何的注入点配成样本。
	s.armed = false
	s.lastInj = nil
}
