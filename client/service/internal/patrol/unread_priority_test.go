...
func TestUnreadClassificationCorrectionStopsInPlace(t *testing.T) {
	...
	if names := h.runner.names(); len(names) != 7 ||
		names[0] != protocol.PrimNavEnsureSurface ||
		names[1] != protocol.PrimChatReadUnreadTotal ||
		names[2] != protocol.PrimChatReadList ||
		names[3] != protocol.PrimChatIdentifyCurrentConversation ||
		names[4] != protocol.PrimChatOpenConversation ||
		names[5] != protocol.PrimChatIdentifyCurrentConversation ||
		names[6] != protocol.PrimChatReadThread {
