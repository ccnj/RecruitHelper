...
func TestDialogueTurnInputKindOfMixedShapes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wechat_card_mix_rejected", inbound: []Message{text(2, "加个微信"), {Seq: 3, Direction: "in", Kind: "card", CardType: "wechatExchange", CardState: "pending", Origin: "external"}}, ok: false},
		{name: "interview_card_mix_rejected", inbound: []Message{{Seq: 2, Direction: "in", Kind: "card", CardType: "interviewInvite", CardState: "accepted", Origin: "external"}, text(3, "到时见")}, ok: false},
