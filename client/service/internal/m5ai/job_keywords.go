...
const JobKeywordsInputFormatVersion = "jobKeywords/1"
...
// jobKeywordsInstruction 是甲方 2026-07-31 审定的提示词原文。
//
...
3. 词库里确实没有合适的，才可以自己写，但最多 2 个。自己写的必须是招聘者用来筛人的短词（技能、行业、背景、证书），不能是句子或短语。
4. 同一个分组里选的词，不能超过该组标明的上限。
5. 必须与职位名称和职位描述所述的实际工作内容相符。不要选描述里找不到依据的词，宁可少选一个也不要硬凑。
6. 必须做出选择。不允许弃选，不允许返回空数组，不允许返回「无法判断」。
