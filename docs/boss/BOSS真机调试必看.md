...
两条附注：
- **调试器横幅会挪页面。** 插件挂上调试器时标签页顶部出一条横幅，页面内容整体下移几十像素。OS 注入的标定是从落点学出来的，横幅出现或消失，下一次落点就偏这么多——闸会拦住并重学一趟，不会点错，只是多花一趟。一次考古里让挂载状态保持稳定：要么先挂上再开始动，要么全程不挂。
