...
| 游标与快照形态 | **脑持会话级游标,手回有界窗口全量快照,新增投影在脑侧做**。游标 = 脑账本的"锚尾"(最近 ≤5 条消息的 方向+内容哈希 序列),随命令下发仅作滚动止损提示;手无任何跨轮记忆。对齐算法 = 在快照任意连续片段中匹配账本后缀、取最大重叠;同长多解取最靠后的起点以最少新增并审计。卡片消息按"身份哈希 + 可变状态分离"处理,按钮事件 = 对齐位置上的状态跃迁,而非新消息。 |
...
1. 求最大的 j,使 S 的某段连续区间恰为 L 的长度 j 后缀(取**最大重叠**)。手在唯一锚命中时通常会裁掉更旧上下文,但重复锚时必须保留全部候选,所以脑不能把“从 S[0] 开始”当正确性前提。
...
4. 歧义(同一最大 j 在 S 中有多个起点,只在候选人贴尾连发逐字相同内容且窗口边界恰好落在其间时可能):取最靠后的起点(最少新增)并写审计。偏置理由:对话轮合并使少量重复无害化,但重复卡片会重触发事件层;而漏消息的风险由锚尾 ≥3 条的上下文携带压到实际为零——单条重复文本("好""好")因锚尾包含前一条不同消息而自然消歧。残余风险登记在 §10。
...
- 对齐用身份哈希 → 老卡永远对得上位置;脑比较**对齐位置上的 cardState 差**:账本 pending、快照 accepted ⇒ 在同一 SQLite 事务中更新 `Message.CardState`，并追加一条跃迁事实 outbox。事实正式键含 platform/accountRef/conversationRef/messageSeq 及 from/to，同时记录 roundId/contentHash/cardType/createdAt。
- 业务事件消费者按 createdAt 读取未确认跃迁，完成下游处理后再显式、幂等 acknowledge；读取不删除，ack 也只记录确认时间而保留追加事实。因此脑在状态提交后、业务处理前崩溃，重启后仍会重新读到同一 pending 事实。
- 去重在两层成立:状态更新入账后,下轮两侧同为 accepted,无跃迁无事件;数据库复合唯一键又阻止完全相同的 from→to 事实重复入账。这天然支持规格要求的"已结束者点旧卡直进已约面"(状态跃迁与档案状态无关,谁跃迁谁触发)。
