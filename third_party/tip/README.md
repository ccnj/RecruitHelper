...
随客户端整包携带,**每台客户机都会落盘**(2026-09-02 甲方裁决:保持随包,不做开关;
执行方已陈述未签名 Rust cdylib + TSF 注册逻辑对杀软启发式的风险,甲方知情接受)。
不注册就是个死文件。但**不留在安装目录**。壳启动时把它安置到:
...
壳会在下次启动时自动把新版换进固定目录(指纹不同才动手)——**客户端升级即换 DLL**,
不需要人碰。两个注意:
